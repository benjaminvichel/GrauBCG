...
int main()
{
	...
	for (int i = 0; i < 3; ++i) // cria 3 objetos para exemplo
	{
		AnimatedObject obj;
		obj.VAO = VAO;
		obj.textureID = texID;
		obj.vertexCount = vertexCount;
		obj.position = glm::vec3(i * 3.0f, 0.0f, 0.0f); // posições diferentes
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (auto &obj : objects)
		{
			...
			{
				glm::vec3 from = obj.waypoints[obj.currentWaypoint];
				glm::vec3 to = obj.waypoints[(obj.currentWaypoint + 1) % obj.waypoints.size()];
				obj.position = glm::mix(from, to, obj.t);
